<form id="payment" class="form-horizontal">
  <fieldset>
    ...
    <div class="form-group required">
        <label class="col-sm-2 control-label" for="input-cc-expire-date"><?php echo $entry_cc_type; ?></label>
        <div class="col-sm-3"> 
            <select name="cc_type" class="form-control">
              <?php foreach ($cc_type as $key_cc => $val_cc) { ?>
              <option value="<?php echo $val_cc; ?>"><?php echo $key_cc; ?></option>
              <?php } ?>
            </select>
        </div>
    </div>
...
$('#button-confirm').on('click', function() {
	$.ajax({
		url: 'index.php?route=payment/velocitycreditcard/send',
		type: 'post',
		data: $('#payment :input'),
		dataType: 'json',
		cache: false,
